"""
Vide les runs d'optimisation (tournees + affectations).

id_run n'est pas une sequence : il est calcule par MAX(id_run)+1 a chaque
POST. Vider ces deux tables suffit donc a faire repartir la numerotation
a 1 au prochain lancement -- aucun compteur a reinitialiser.
...
def main() -> None:
    ...
    try:
        ...
        db.commit()
        print(f"supprime : {nb_aff} affectations, {nb_tour} tournees")
