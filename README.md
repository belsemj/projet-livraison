...
data/seed.sql est la source de vérité des coordonnées. Les fichiers rapport_geocodage.csv, regeocodage_controle.csv et echantillon_routier.csv sont des traces d'analyse : ils reflètent l'état à leur date de production et ne doivent pas être mis à jour.

D18 : Endpoint agrégé GET /runs/{id_run} (tournées + affectations
...
D37 — Prédiction de durée : brique méthodo sur données simulées. Faute de durées historiques (Q3), la prédiction est traitée comme démonstration de pipeline, pas comme service. Modèle génératif documenté (vitesse, service/arrêt, min/m³, supplément caisson, congestion douce) ; features [distance, nb_arrêts, volume, part_caisson_spécial] ; bruit gaussien fixe. Régression linéaire retenue (relation quasi-linéaire, distance ≈ 98 % de l'importance), RandomForest en garde-fou. Aucun endpoint de prédiction en prod (un modèle calé sur du simulé donnerait de faux chiffres). Cible à remplacer par les vraies durées, pipeline inchangé.
